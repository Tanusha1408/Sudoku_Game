...
def Safetoprint(grid, row, col, num):
...
# Takes a partially filled-in grid and attempts to assign values to all unassigned locations in such a way to meet the requirements
# for Sudoku solution (non-duplication across rows, columns, and boxes)

def solvingSudoku(grid, row, col):
	...
	for num in range(1, N + 1, 1):
		...
		# move to next column
		if Safetoprint(grid, row, col, num):
...
# 0 means unassigned cells which are empty
...
#Implementation by Team-7 DSA CAPSTONE
